...
**Clamping a one-character field to the first grapheme makes the emoji keyboard look dead.** A `TextField` *appends*, so with 🛒 already in the section icon field, tapping 🍳 delivers `"🛒🍳"` to `onValueChange` — and taking the first grapheme cluster faithfully keeps 🛒. Every tap appeared to do nothing. Fix is `nextIconInput`: if the new text is the old text plus something, that something is what the user just chose and wins; anything else (typing over a selection, pasting, deleting) is still read from the front so `"🛒 shopping"` resolves to 🛒. Unit tests missed this entirely because they only ever exercised an empty field — it took driving the real dialog to see it.
...
- **The `⋮` menu carries no "Edit text".** Tapping the text already does it, and a menu entry for a gesture that's always available is just a longer route to the same place. The menu is for what the gestures *can't* reach: mark done, add/edit timer, move to section, delete.
...
- **A link row has no edit mode.** Tapping opens the page, which is the only thing there is to do with a URL; it used to fall back to editing the raw text on long-press, which meant a gesture that existed solely to let you hand-correct a URL. `LinkContent` takes `onOpen` and nothing else.
